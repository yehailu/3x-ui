...
```bash
bash <(curl -Ls https://raw.githubusercontent.com/yehailu/3x-ui/master/install.sh)
```

To install a specific version, append its tag (e.g. `v3.7.0`):

```bash
bash <(curl -Ls https://raw.githubusercontent.com/yehailu/3x-ui/master/install.sh) v3.7.0
```

To install the rolling **dev** build (latest per-commit pre-release from `main`, not a stable release), pass `dev-latest`:

```bash
bash <(curl -Ls https://raw.githubusercontent.com/yehailu/3x-ui/master/install.sh) dev-latest
```
